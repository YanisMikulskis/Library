...
Чтобы запустить мой гит репозиторий (или же другое виртуальное окружение) нужно сначала подключиться 
к интерпретатору в этом виртуальном окружении. Сначала запускается питон внутри окружения, а потом все 
пакеты "вокруг" него. Вполне вероятно, что после клонирования репозитория, интерпретатор сам встанет в окружение.
